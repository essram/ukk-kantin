...
export const createUser = async (request: Request, response: Response) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (role === "ADMIN_STAN") {
        await tx.admin_stan.create({
          data: {
            id_user: newUser.id,
            nama: name,
            nama_pemilik: name, // atau field terpisah
            telp: ""
          }
        })
      }
    ...
    return response.status(200).json({
      status: true,
      data: result,
      message: "New user has created with profile"
    })
...
export const authentication = async (request: Request, response: Response) => {
    try {
        const { email, password, role_tab } = request.body /** get requested data (data has been sent from request) */
        ...
        if (!findUser) return response
            ...
            if (role_tab && findUser.role !== role_tab) {
                return response
                    .status(200)
                    .json({ status: false, logged: false, message: `You are not allowed to login as ${role_tab}` })
            }
        ...
        //     data = {
        //         ...data,
        //         admin_stan: findUser.admin_stan
        //     }
        // }
