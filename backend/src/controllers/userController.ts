...
export const createUser = async (request: Request, response: Response) => {
  try {
    const { name, email, alamat, telp, password, nama_pemilik, role } =
      request.body;
    const uuid = uuidv4();
      ...
      if (role === "ADMIN_STAN") {
        await tx.admin_stan.create({
          data: {
            id_user: newUser.id,
            nama: name,
            nama_pemilik: nama_pemilik || name,
            telp: telp || "",
          },
        });
      }
...
export const deleteUser = async (request: Request, response: Response) => {
  try {
    ...
    if (exists && findUser.profile_picture !== ``)
      fs.unlinkSync(path); /** if file exist, then will be delete */
  ...
  try {
    ...
    if (role_tab && findUser.role !== role_tab) {
      return response.status(200).json({
        status: false,
        logged: false,
        message: `You are not allowed to login as ${role_tab}`,
      });
    }
