...
export const getFavourite = async (request: Request, response: Response) => {
    try {
        ...
        transaction_details.forEach(transaction_detail => {
            const menuName = transaction_detail.menu?.name; // Nama menu
            if (menuName) {
                if (!menuCount[menuName]) {
                    menuCount[menuName] = 0; // Inisialisasi jika belum ada
                }
                menuCount[menuName] += transaction_detail.quantity; // Menambahkan jumlah pemesanan
            }
        });
